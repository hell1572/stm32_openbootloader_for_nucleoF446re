...
OPENBL_MemoryTypeDef FLASH_Descriptor =
{
  FLASH_START_ADDRESS,
  FLASH_END_ADDRESS,
  (512U * 1024U), //FLASH_BL_SIZE,
  FLASH_AREA,
  OPENBL_FLASH_Read,
  OPENBL_FLASH_Write,
  OPENBL_FLASH_SetReadOutProtectionLevel,
  NULL, //OPENBL_FLASH_SetWriteProtection,
...
void OPENBL_FLASH_Write(uint32_t Address, uint8_t *pData, uint32_t DataLength)
{
  ...
  // if (length <= 8U)
  // {
    // Write as words if data is double word or less
    for (index = 0U; index < length; index += 4U)
    {
      uint32_t word = *((uint32_t *)(pData + index));
      OPENBL_FLASH_ProgramWord(Address + index, word);
    }
  // }
  // else
  // {
  //   // Write as double words for larger data
  //   for (index = 0U; index < length; index += 8U)
  //   {
  //     uint64_t dword = *((uint64_t *)(pData + index));
  //     OPENBL_FLASH_ProgramDoubleWord(Address + index, dword);
  //   }
  // }
...
ErrorStatus OPENBL_FLASH_Erase(uint8_t *p_Data, uint32_t DataLength)
{
  ...
  // erase_init_struct.VoltageRange = FLASH_VOLTAGE_RANGE_3; // Assuming VDD = 3.3V
  ...
  for (counter = 0U; ((counter < pages_number) && (counter < (DataLength / 2U))) ; counter++)
  {
    ...
    // // else if (erase_init_struct.Sector <= 255)
    // // {
    // //   erase_init_struct.Banks = FLASH_BANK_2;
    // // // }
    // else
...
/* Private functions ---------------------------------------------------------*/
/**
  * @brief  Program word at a specified FLASH address.
...
static ErrorStatus OPENBL_FLASH_EnableWriteProtection(uint8_t *ListOfPages, uint32_t Length)
{
  ...
  //   flash_ob.WRPSector = (wrp_start_offset << 8) | wrp_end_offset;  
  //   flash_ob.WRPState = OB_WRPSTATE_ENABLE;
  //   flash_ob.Banks = FLASH_BANK_1;
  //   flash_ob.RDPLevel = OB_RDP_LEVEL_0;
...
static ErrorStatus OPENBL_FLASH_DisableWriteProtection(void)
{
  ...
  // flash_ob.WRPSector = 0xFFFF; // Disable all write protection sectors
